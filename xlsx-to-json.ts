...
interface Arguments {
  ...
  keepFods?: boolean;
  blacklistFile?: string; // Path to blacklist file
...
async function convertXlsxToTsModules() {
  const argv = yargs(hideBin(process.argv))
    ...
    .option('blacklistFile', {
      type: 'string',
      description: 'Path to blacklist file (one sheet name per line, # for comments). Default: xlsx-blacklist.txt',
      default: 'xlsx-blacklist.txt',
    })
  ...
  // Read blacklist file (if exists)
  const blacklistSet = new Set<string>();
  const blacklistFilePath = argv.blacklistFile ? path.resolve(argv.blacklistFile) : null;
  if (blacklistFilePath && fs.existsSync(blacklistFilePath)) {
    const blacklistContent = fs.readFileSync(blacklistFilePath, 'utf-8');
    blacklistContent.split('\n').forEach(line => {
      const trimmed = line.trim();
      if (trimmed && !trimmed.startsWith('#')) {
        blacklistSet.add(trimmed);
      }
    });
    console.log(`Blacklist: ${blacklistSet.size} sheet(s) excluded from master.ts`);
  }
  ...
  try {
    ...
    try {
      const result = execSync(loCommand, {stdio: 'pipe', encoding: 'utf8'});
      console.log('Conversion to FODS successful.');
      if (result && result.trim()) {
        console.log('LibreOffice output:', result);
      }
    } catch (e: any) {
      // Check if it's just a warning (file still gets converted)
      if (fs.existsSync(tempFodsFile)) {
        console.warn('⚠  LibreOffice conversion completed with warnings:');
        console.warn('Error output:', e.stderr?.toString() || e.message);
        console.warn('This is likely due to column count warnings and can be safely ignored.');
        console.log('Conversion to FODS successful (with warnings).');
      } else {
    ...
    workbook.eachSheet((worksheet) => {
      ...
      if (blacklistSet.has(originalSheetName)) {
        sheetFileContent += `// NOTE: This sheet is blacklisted and not included in master.ts\n`;
      }
    ...
    sheetFileInfos.forEach(info => {
      if(blacklistSet.has(info.originalName)) masterFileContent += '// (excluded by blacklist) ' ;
