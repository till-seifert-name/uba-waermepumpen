...
export class GebaeudeHeatingComponent {
  ...
  onSubmit(): void {
    ...
    // Wenn Nachtspeicherheizung oder Andere ausgewählt wurde, zu speziellem Feedback navigieren
    if (heizungstyp === Daten['E60'] || heizungstyp === Daten['E61']) { // Nachtspeicherheizung oder Andere
      this.router.navigate(['/gebaeude/feedback-heizung']);
    } else {

      // Check if building type is Mehrfamilienhaus - skip flow temperature page
      const gebaeudetyp = this.grid.getCell('IN_build', 'P3');
