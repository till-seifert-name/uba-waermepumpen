...
export class WizardTabsComponent implements OnInit, OnDestroy {
  ...
  baseTabs: TabConfig[] = [
    {id: 'gebaeude', title: 'Gebäude', routePath: '/gebaeude', pathMatch: /\/gebaeude(?!\/raeume|\/ergebnis)/, icon: 'house'},
    {id: 'raeume', title: 'Räume', routePath: '/gebaeude/raeume/liste', pathMatch: /\/gebaeude\/raeume.*/, icon: 'grid'},
    {id: 'ergebnis', title: 'Ergebnis', routePath: '/gebaeude/ergebnis', pathMatch: /\/gebaeude\/ergebnis.*/, icon: 'check-circle'}
  ];
  ...
  updateActiveTabFromUrl(): void {
    ...
    if (this.currentRoomId) {
      for (const tab of this.tabs) {
        if (tab.roomId === this.currentRoomId && this.router.url.match(tab.pathMatch)) {
    ...
    for (const tab of this.tabs) {
      if (this.router.url.match(tab.pathMatch)) {
        this.activeTabId = tab.id;
  ...
  updateRoomTabs() {
    ...
    this.rooms.forEach((room, index) => {
      ...
      this.roomTabs.push({
        id: `room_${room.id}`,
        title: room.name,
        routePath: '/gebaeude/raeume/detail-basis',
        queryParams: {room: room.id},
        pathMatch: /\/gebaeude\/raeume\/detail.*/,
        roomId: room.id,
