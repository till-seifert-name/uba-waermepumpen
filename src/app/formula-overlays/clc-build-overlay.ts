...
export class ClcBuildOverlay implements FormulaOverlay {
  ...
  applyFormulas(grid: DataGrid): void {
    ...
    for (let i = 0; i < CLC_BUILD_COLS.length; i++) {
       ...
       * Excel: "IFERROR(ROUND(clc_load!I74/HLOOKUP(\"NO_round_Qdot\", PAR!$F$1:$AZ$5, 2, FALSE),0)*HLOOKUP(\"NO_round_Qdot\", PAR!$F$1:$AZ$5, 2, FALSE),\"\")"
       */
      grid.setCell('clc_build', `${clcBuildCol}7`, (s, c, g) => {
        const noRoundQdot = g.HLOOKUP('PAR', 'NO_round_Qdot', '$F$1', '$AZ$5', 2, false) as number;
        return g.IFERROR(Math.round(g.n('clc_load', `${clcLoadCol}74`) / noRoundQdot) * noRoundQdot, "");
      });
      ...
      /**
       * Row 41: IST 55°C - Fehlende Deckung
       * Excel: "IF(G$45,(MAX(0,1-G39)),\"\")"
       */
      grid.setCell('clc_build', `${clcBuildCol}41`, (s, c, g) =>
        g.WENN(
          g.n(s, `${clcBuildCol}45`) !== 0,
          g.MAX(0, 1 - g.n(s, `${clcBuildCol}39`)),
          ""
        ));

      /**
       * Row 42: Typ 33 55°C - Fehlende Deckung
       * Excel: "IF(G$45,(MAX(0,1-G40)),\"\")"
       */
      grid.setCell('clc_build', `${clcBuildCol}42`, (s, c, g) =>
        g.WENN(
          g.n(s, `${clcBuildCol}45`) !== 0,
          g.MAX(0, 1 - g.n(s, `${clcBuildCol}40`)),
          ""
        ));
    ...
    /**
     * Row 81: Fehlende Deckung bei 55°C - Heizlast gewichtetes Mittel der Abweichung IST-Heizkörper
     * Excel: "SUMPRODUCT(G7:AA7,G41:AA41)/SUM(G7:AA7)"
     */
    grid.setCell('clc_build', 'G81', (s, c, g) => {
      const heizlast = g.getCellRange(s, 'G7:AA7');
      const fehlendeDeckung = g.getCellRange(s, 'G41:AA41');
      return g.SUMPRODUCT(heizlast, fehlendeDeckung)
        / g.SUM(s, g.getCellRange(s, 'G7:AA7'));
    });

    /**
     * Row 81: Fehlende Deckung bei 55°C - Heizlast gewichtetes Mittel der Abweichung Typ 33
     * Excel: "SUMPRODUCT(G7:AA7,G42:AA42)/SUM(G7:AA7)"
     */
    grid.setCell('clc_build', 'H81', (s, c, g) => {
      const heizlast = g.getCellRange(s, 'G7:AA7');
      const fehlendeDeckungTyp33 = g.getCellRange(s, 'G42:AA42');
      return g.SUMPRODUCT(heizlast, fehlendeDeckungTyp33)
        / g.SUM(s, g.getCellRange(s, 'G7:AA7'));
    });

    // NEW Typ 33 replacement heater building-scale calculations (H column)
    // These are the new formulas added for log entry F118

    /**
     * Row 65 (H column): Anzahl Räume Deckungsgrad Typ 33 < 0,6
     * Excel: "COUNTIF($G$40:$AA$40,\"<0,6\")"
     */
    grid.setCell('clc_build', 'H65', (s, c, g) =>
      g.COUNTIF(s, 'G40:AA40', '<0.6'));

    /**
     * Row 66 (H column): Anzahl Räume Deckungsgrad Typ 33 < 0,8
     * Excel: "COUNTIF($G$40:$AA$40,\"<0,8\")"
     */
    grid.setCell('clc_build', 'H66', (s, c, g) =>
      g.COUNTIF(s, 'G40:AA40', '<0.8'));

    /**
     * Row 67 (H column): Anzahl Räume 0,6<Deckungsgrad Typ 33<0,8
     * Excel: "COUNTIFS(G40:AA40,\">=0,6\",G40:AA40,\"<0,8\")"
     */
    grid.setCell('clc_build', 'H67', (s, c, g) =>
      g.COUNTIFS(s, 'G40:AA40', '>=0.6', 'G40:AA40', '<0.8'));

    /**
     * Row 68 (H column): Anzahl Räume Deckungsgrad Typ 33 < 1
     * Excel: "COUNTIF(G40:AA40,\"<1\")"
     */
    grid.setCell('clc_build', 'H68', (s, c, g) =>
      g.COUNTIF(s, 'G40:AA40', '<1'));

    /**
     * Row 69 (H column): Anzahl Räume 0,8<Deckungsgrad Typ 33<1,0
     * Excel: "COUNTIFS(G40:AA40,\">=0,8\",G40:AA40,\"<1\")"
     */
    grid.setCell('clc_build', 'H69', (s, c, g) =>
      g.COUNTIFS(s, 'G40:AA40', '>=0.8', 'G40:AA40', '<1'));

    /**
     * Row 70 (H column): Anzahl Räume Deckungsgrad Typ 33 > 1
     * Excel: "COUNTIF(G40:AA40,\">=1\")"
     */
    grid.setCell('clc_build', 'H70', (s, c, g) =>
      g.COUNTIFS(s, 'G40:AA40', '>=1'));

    /**
     * Row 73 (H column): Anteil Räume Deckungsgrad Typ 33 < 0,6
     * Excel: "H65/$G$49"
     */
    grid.setCell('clc_build', 'H73', (s, c, g) =>
      g.n(s, 'H65') / g.n(s, 'G49'));

    /**
     * Row 74 (H column): Anteil Räume Deckungsgrad Typ 33 < 0,8
     * Excel: "H66/$G$49"
     */
    grid.setCell('clc_build', 'H74', (s, c, g) =>
      g.n(s, 'H66') / g.n(s, 'G49'));

    /**
     * Row 75 (H column): Anteil Räume 0,6<Deckungsgrad Typ 33<0,8
     * Excel: "H67/$G$49"
     */
    grid.setCell('clc_build', 'H75', (s, c, g) =>
      g.n(s, 'H67') / g.n(s, 'G49'));

    /**
     * Row 76 (H column): Anteil Räume Deckungsgrad Typ 33 < 1
     * Excel: "H68/$G$49"
     */
    grid.setCell('clc_build', 'H76', (s, c, g) =>
      g.n(s, 'H68') / g.n(s, 'G49'));

    /**
     * Row 77 (H column): Anteil Räume 0,8<Deckungsgrad Typ 33<1
     * Excel: "H69/$G$49"
     */
    grid.setCell('clc_build', 'H77', (s, c, g) =>
      g.n(s, 'H69') / g.n(s, 'G49'));

    /**
     * Row 78 (H column): Anteil Räume Deckungsgrad Typ 33 > 1
     * Excel: "H70/$G$49"
     */
    grid.setCell('clc_build', 'H78', (s, c, g) =>
      g.n(s, 'H70') / g.n(s, 'G49'));
