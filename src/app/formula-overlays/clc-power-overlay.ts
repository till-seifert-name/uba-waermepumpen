    ...
    for (let i = 0; i < CLC_POWER_COLS.length; i++) {
      ...
      grid.setCell('clc_power', `${clcPowerCol}18`, (s, c, g) => {
        ...
        return g.WENNS(
          ...
          typ === "Stahlradiator", (() => {
            // Excel: INDEX(Daten!$AF$13:$AF$23,XMATCH(MIN(IF(Daten!$AE$13:$AE$23=G17,ABS(Daten!$AF$13:$AF$23-IN_rooms!R$53))),
            //                                        IF(Daten!$AE$13:$AE$23=G17,ABS(Daten!$AF$13:$AF$23-IN_rooms!R$53)),0))
            const sizeArray = g.RANGE("Daten!AE13:AE23", s);
            const valueArray = g.RANGE("Daten!AF13:AF23", s).map(Number);
            const condition = g.GLEICH(sizeArray, size);
            const ifArray = g.WENN_ARRAY(condition, g.ABS_ARRAY(valueArray.map(v => v - radius)), []);
            const minValue = g.MIN(ifArray); // Let MIN handle Excel filtering rules
            const matchIndex = g.XMATCH(minValue, ifArray, 0);
            return g.INDEX_ARRAY(valueArray, matchIndex) || 0; // Excel INDEX with 1-based position from XMATCH
          })(),

          typ === "Gussradiator", (() => {
            // Excel: INDEX(Daten!$AD$13:$AD$25,XMATCH(MIN(IF(Daten!$AC$13:$AC$25=G17,ABS(Daten!$AD$13:$AD$25-IN_rooms!R$53))),
            //                                        IF(Daten!$AC$13:$AC$25=G17,ABS(Daten!$AD$13:$AD$25-IN_rooms!R$53)),0))
            const sizeArray = g.RANGE("Daten!AC13:AC25", s);
            const valueArray = g.RANGE("Daten!AD13:AD25", s).map(Number);
            const condition = g.GLEICH(sizeArray, size);
            const ifArray = g.WENN_ARRAY(condition, g.ABS_ARRAY(valueArray.map(v => v - radius)), []);
            const minValue = g.MIN(ifArray); // Let MIN handle Excel filtering rules
            const matchIndex = g.XMATCH(minValue, ifArray, 0);
      ...
      grid.setCell('clc_power', `${clcPowerCol}26`, (s, c, g) => {
        ...
        const höhe = g.XVERWEIS(
          ...
          g.getCells('Data_radiator', 'G5', 'G32').flat(),
          { ifNotFound: 1000000, matchMode: 'exactOrNextLarger', searchMode: 'first' }
        );
        const tiefe = 155;
      ...
      /**
       * Row 30: Width calculation for Fensterbankradiator or direct reference
       * Excel: "IF(G28=\"Fensterbankradiator\",
      ...
      grid.setCell('clc_power', `${clcPowerCol}30`, (s, c, g) => {
        if (g.g(s, `${clcPowerCol}28`) === "Fensterbankradiator") {
          const userWidth = g.n('IN_rooms', `${roomCol}52`);
          ...
          // Excel: INDEX(Fensterbankradiator_b[],MATCH(MIN(ABS(Fensterbankradiator_b[]-IN_rooms!R$52)),ABS(Fensterbankradiator_b[]-IN_rooms!R$52),0))
          const differences = g.ABS_ARRAY(radiatorWidths.map(width => width - userWidth));
          const minValue = g.MIN(differences);
          const matchIndex = g.XMATCH(minValue, differences, 0);

          return g.INDEX_ARRAY(radiatorWidths, matchIndex) || 0;
        } else {
          return g.g('IN_rooms', `${roomCol}52`);
        }
      });

      /**
       * Row 31: Depth from IN_rooms
       * Excel: "IN_rooms!S$53"
       */
       ...
       * Excel: "(G$35-G$36)/(LN((G$35-clc_load!I$10)/(clc_power!G$36-clc_load!I$10)))"
       */
       ...
       * Excel: "IF(IN_rooms!R$63=\"Rohrradiator\",IN_rooms!R$63,\"\")"
       */
       ...
       * Excel: "IF(G161=\"Fensterbankradiator\",
      ...
      grid.setCell('clc_power', `${clcPowerCol}163`, (s, c, g) => {
        if (g.g(s, `${clcPowerCol}161`) === "Fensterbankradiator") {
          ...
          // Excel: INDEX(Fensterbankradiator_b[],MATCH(MIN(ABS(Fensterbankradiator_b[]-IN_rooms!R$59)),ABS(Fensterbankradiator_b[]-IN_rooms!R$59),0))
          const differences = g.ABS_ARRAY(radiatorWidths.map(width => width - userWidth));
          const minValue = g.MIN(differences);
          const matchIndex = g.XMATCH(minValue, differences, 0);

          return g.INDEX_ARRAY(radiatorWidths, matchIndex) || 0; // Excel INDEX with 1-based position from XMATCH
        } else {
