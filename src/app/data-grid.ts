...
export class DataGrid {
  ...
  serializeWhitelistedCells(parsedReferences: [string, string][], indent = false): string {
    ...
    return JSON.stringify(whitelistedSheets, null, indent ? 2 : 0);
  }
  ...
  setCell(sheet: string, cell: string, value: CellContent): void {
    ...
    if(this.cells[sheet]?.[cell] === value) return;
  ...
  AVERAGE(sheet: string, cellsOrValues: (string | number)[]): number {
    ...
    for (const item of cellsOrValues) {
      let v: any = item;
      if (typeof item === 'string') {
        // treat as a single cell ref if it looks like one; otherwise try numeric literal
        if (item.match(/^[A-Za-z]+\d+$/)) v = this.getCell(sheet, item);
      }
      if (v === '' || v === null || v === undefined) continue;           // ignore empties
      if (typeof v === 'number' && Number.isFinite(v)) values.push(v);     // numbers ok
      else if (typeof v === 'string' && this.isExcelNumericString(v)) {  // numeric text from literal
        const n = Number(v);
  ...
  /**
   * Array-based INDEX function - returns the value at a specific position in an array
   * Excel: INDEX(array, position) where position is 1-based
   * @param array The array to index into
   * @param position The 1-based position to return (Excel uses 1-based indexing)
   * @returns The value at the specified position
   */
  INDEX_ARRAY<T>(array: T[], position: number): T | null {
    if (position < 1 || position > array.length) {
      return null; // Excel behavior for out-of-bounds
    }
    return array[position - 1]; // Convert to 0-based for JavaScript
  }
  ...
  WENN<T extends number | string | boolean | null, U extends number | string | boolean | null>(
    cond: boolean|number|string, thenVal: T, elseVal: U): T | U {
  ...
  /**
   * Excel IFERROR function - returns fallback value if the first value is an error (NaN)
   */
  IFERROR<T extends number | string | boolean | null, U extends number | string | boolean | null>(
    value: T, fallback: U): T | U {
    return (typeof value === 'number' && isNaN(value)) ? fallback : value;
  }

  /**
   * Excel ROUND function - rounds a number to a specified number of digits
   */
   ...
   * Implementation of Excel's MAX function with correct Excel behavior:
   * - Ignores TRUE/FALSE unless all entries are logicals (then TRUE=1, FALSE=0)
   * - Ignores text unless it's a numeric string and is a literal argument
   * - Ignores error values unless all entries are errors
   */
  MAX(...values: (any | any[])[]): number {
    const flatValues = values.flat();

    if (flatValues.length === 0) return 0;

    // Check if all values are logicals (boolean)
    const allLogicals = flatValues.every(v => typeof v === 'boolean');
    if (allLogicals) {
      const logicalNumbers = flatValues.map(v => v ? 1 : 0);
      return Math.max(...logicalNumbers);
    }

    // Check if all values are errors (for this implementation, we'll consider NaN as error)
    const allErrors = flatValues.every(v =>
      (typeof v === 'number' && isNaN(v)) || v === null || v === undefined
    );
    if (allErrors) {
      return NaN; // Return error if all are errors
    }

    // Filter to only numbers, ignoring booleans, text, and errors
    const numericValues = flatValues.filter(v => {
      // Include actual numbers that are not NaN
      if (typeof v === 'number' && !isNaN(v)) return true;

      // Include numeric strings if they're literal arguments (we'll treat all numeric strings as valid)
      if (typeof v === 'string') {
        return this.isExcelNumericString(v) && isFinite(Number(v));
      }

      return false;
    }).map(v => typeof v === 'number' ? v : Number(v));

    // If no numeric values found, return 0 (Excel behavior)
   ...
   * Implementation of Excel's MIN function with correct Excel behavior:
   * - Ignores TRUE/FALSE unless all entries are logicals (then TRUE=1, FALSE=0)
   * - Ignores text unless it's a numeric string and is a literal argument
   * - Ignores error values unless all entries are errors
   */
  MIN(...values: (any | any[])[]): number {
    const flatValues = values.flat();
    ...
    if (allLogicals) {
      const logicalNumbers = flatValues.map(v => v ? 1 : 0);
      return Math.min(...logicalNumbers);
    }

    // Check if all values are errors (NaN as error)
    const allErrors = flatValues.every(v =>
      (typeof v === 'number' && isNaN(v)) || v === null || v === undefined
    );
    if (allErrors) {
   ...
   * Implementation of Excel's XMATCH function with strict type matching rules:
   * - Numbers vs numeric text: No auto-coercion (123 != "123")
   * - Booleans: Must match exactly (TRUE != 1, FALSE != 0)
   * - Empty cells: Match "" for text searches, 0 only for explicit numeric 0
   ...
   * @returns The position (1-based) of the matched value or 0 if not found
   */
  XMATCH(
    lookupValue: string | number | boolean,
    lookupArray: (string | number | boolean | null | undefined)[],
    matchType: number = 0,
    searchType: number = 1
  ): number {
    ...
    for (const i of indices) {
      ...
      // Exact match case with strict Excel type checking
      if (matchMode === 'exact') {
        if (this.excelStrictEquals(lookupValue, currentValue)) {
          return i + 1; // Excel uses 1-based indexing
        }
      }

      // Next smaller/larger match cases (only work with numbers)
      if (typeof lookupValue === 'number' && typeof currentValue === 'number') {
        if (matchMode === 'exactOrNextSmaller' && currentValue <= lookupValue) {
          if (currentValue === lookupValue) return i + 1; // Exact match
  ...
  /**
   * Excel-strict equality comparison for XMATCH
   * - Numbers vs numeric text: 123 !== "123"
   * - Booleans: TRUE !== 1, FALSE !== 0
   * - Empty/null: null/undefined can match "" in text context
   */
  private excelStrictEquals(a: any, b: any): boolean {
    // Handle null/undefined/empty cases
    if (a === null || a === undefined) a = '';
    if (b === null || b === undefined) b = '';

    // Exact type and value match required
    if (typeof a !== typeof b) return false;

    // For numbers, use strict equality (handles NaN, +0/-0 correctly)
  ...
  WENN_ARRAY(condition: boolean[], ifTrue: number[], ifFalse: number[] = []): (number | boolean)[] {
    return condition.map((c, i) => c ? ifTrue[i] : (ifFalse[i] ?? false));
  }
  ...
  COUNTIF(sheet: string, range: string, criteria: string | number |boolean): number {
    ...
    for (const cell of cells) {
      // Don't force numeric conversion - preserve original cell types for Excel compatibility
      // Handle undefined cells as empty strings (Excel behavior)
   ...
   */
  COUNTIFS(sheet: string, ...args: (string | number | boolean)[]): number {
    // Args come in pairs: range1, criteria1, range2, criteria2, etc.
    if (args.length % 2 !== 0) return 0;

    const ranges: any[][] = [];
    const criterias: (string | number | boolean)[] = [];
    ...
    for (let i = 0; i < length; i++) {
      ...
      for (let j = 0; j < ranges.length; j++) {
        // Don't force numeric conversion - preserve original cell types
        const cellValue = ranges[j][i] ?? '';
        if (!this.matchesCriteria(cellValue, criterias[j])) {
          allMatch = false;
          break;
        }
      }
   ...
   * Supports mixed data types, wildcards, and all Excel comparison operators
   */
  private matchesCriteria(value: any, criteria: string | number | boolean): boolean {
    // Handle undefined/null values as empty strings (Excel behavior)
    const cellValue = value ?? '';

    // Direct type-based matching for numeric and boolean criteria
    if (typeof criteria === 'number') {
      // Excel: numbers only match numbers with exact equality
      return typeof cellValue === 'number' && cellValue === criteria;
    }

    if (typeof criteria === 'boolean') {
      // Excel: boolean criteria match numbers (true=1, false=0) and booleans
      if (typeof cellValue === 'number') {
        return criteria ? cellValue === 1 : cellValue === 0;
      }
      return typeof cellValue === 'boolean' && cellValue === criteria;
    }

    // String criteria processing
    const criteriaStr = criteria.toString().trim();

    // Parse comparison operators (must check longer operators first)
    if (criteriaStr.startsWith('>=')) {
      return this.compareNumeric(cellValue, Number(criteriaStr.substring(2)), '>=');
    } else if (criteriaStr.startsWith('<=')) {
      return this.compareNumeric(cellValue, Number(criteriaStr.substring(2)), '<=');
    } else if (criteriaStr.startsWith('<>')) {
      // Not equal operator - supports both numeric and string comparison
      const compareValue = criteriaStr.substring(2);
      return !this.matchesExact(cellValue, compareValue);
    } else if (criteriaStr.startsWith('>')) {
      return this.compareNumeric(cellValue, Number(criteriaStr.substring(1)), '>');
    } else if (criteriaStr.startsWith('<')) {
      return this.compareNumeric(cellValue, Number(criteriaStr.substring(1)), '<');
    } else if (criteriaStr.startsWith('=')) {
      // Explicit equality
      ...
      // Default case: check for wildcards or exact match
      return this.matchesPattern(cellValue, criteriaStr);
    }
  }

  /**
   * Numeric comparison helper for COUNTIF operators
   * Implements Excel's number coercion rules for numeric strings
   */
  private compareNumeric(cellValue: any, criteriaNumber: number, operator: string): boolean {
    ...
    // Excel number coercion: numeric strings can be compared with numbers
    else if (typeof cellValue === 'string' && this.isExcelNumericString(cellValue)) {
      numericCellValue = Number(cellValue);
      if (isNaN(numericCellValue)) {
        return false;
      }
    }
    // Non-numeric values cannot be compared numerically
    else {
      return false;
    }

    switch (operator) {
      case '>': return numericCellValue > criteriaNumber;
      case '>=': return numericCellValue >= criteriaNumber;
      case '<': return numericCellValue < criteriaNumber;
      case '<=': return numericCellValue <= criteriaNumber;
  ...
  /**
   * Helper function to determine if a string can be coerced to a number according to Excel's rules
   * Excel allows: digits, leading zeros, single decimal point, minus sign at start
   * Examples: "123", "0.5", "-42", "007", ".5" are valid
   * Examples: "", "abc", "1.2.3", "1-2" are invalid
   */
  private isExcelNumericString(value: string): boolean {
    if (typeof value !== 'string' || value === '') {
      return false;
    }

    // Trim whitespace (Excel does this)
    const trimmed = value.trim();
    if (trimmed === '') {
      return false;
    }

    // Excel numeric pattern: optional minus, digits with optional single decimal point
    // ^-?             : optional minus sign at start
    // (\d+\.?\d*      : digits followed by optional decimal point and optional digits
    //  |              : OR
    //  \.\d+)         : decimal point followed by required digits
    // $               : end of string
    const excelNumericPattern = /^-?(\d+\.?\d*|\.\d+)$/;

    return excelNumericPattern.test(trimmed);
  }

  /**
   * Exact matching helper that preserves type distinctions
   * Implements Excel's number coercion rules for numeric strings
   */
  private matchesExact(cellValue: any, criteriaValue: string): boolean {
    // Try numeric comparison first if criteria looks like a number
    const criteriaNum = Number(criteriaValue);
    if (!isNaN(criteriaNum) && criteriaValue.trim() !== '') {
      // Criteria is a valid number - check if cell can be coerced
      if (typeof cellValue === 'number') {
        return cellValue === criteriaNum;
      }

      // Excel number coercion: check if cell string is truly numeric
      if (typeof cellValue === 'string' && this.isExcelNumericString(cellValue)) {
        const cellNum = Number(cellValue);
        return !isNaN(cellNum) && cellNum === criteriaNum;
      }

      // Cell is not numeric, so no match
      return false;
    }

    // String comparison (convert cell value to string)
    const cellStr = cellValue === null || cellValue === undefined ? '' : cellValue.toString();
    return cellStr === criteriaValue;
  }

  /**
   * Pattern matching helper for wildcards (* and ?)
   */
  private matchesPattern(cellValue: any, pattern: string): boolean {
    // Convert cell value to string for pattern matching
    const cellStr = cellValue === null || cellValue === undefined ? '' : cellValue.toString();

    // Check if pattern contains wildcards
    if (pattern.includes('*') || pattern.includes('?')) {
      return this.matchWildcard(cellStr, pattern);
    }

    // No wildcards - try exact match with type preservation
    return this.matchesExact(cellValue, pattern);
  }

  /**
   * Wildcard matching implementation (* = any characters, ? = single character)
   */
  private matchWildcard(text: string, pattern: string): boolean {
    // Handle special case: single * matches any non-empty text
    if (pattern === '*') {
      return text !== '' && typeof text === 'string';
    }

    // Handle special case: single ? matches exactly one character
    if (pattern === '?') {
    ...
    try {
      const regex = new RegExp(regexPattern, 'i'); // Case insensitive like Excel
   ...
   */
  public getCellRange(sheet: string, range: string): any[] {
